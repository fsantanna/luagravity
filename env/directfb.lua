...
local buffer    = dfb:CreateInputEventBuffer(directfb.DICAPS_ALL, true)
local timeofday = directfb.gettimeofday
...
function directfb.nextEvent ()
    if not buffer:HasEvent() then
        ...
        -- INPUT
        if cls == directfb.DFEC_INPUT
        then
            -- KEYBOARD
	        if tp == directfb.DIET_KEYPRESS then
		        local key = key_map[evt.key_symbol]
		        if _G.warn(key, 'invalid key') then
                    return 'key.press.'..key, key
		        end
	        elseif tp == directfb.DIET_KEYRELEASE then
		        local key = key_map[evt.key_symbol]
		        if _G.warn(key, 'invalid key') then
            ...
            -- MOUSE
	        elseif tp == directfb.DIET_AXISMOTION then
		        if evt.axis == 0 then
                    return 'mouse.x', evt.axisabs
		        else
                    return 'mouse.y', evt.axisabs
		        end
	        elseif tp == directfb.DIET_BUTTONPRESS then
                return 'mouse.press.'..mouse_map[evt.button]
	        elseif tp == directfb.DIET_BUTTONRELEASE then
        ...
        -- USER
        elseif cls == directfb.DFEC_USER then
            error 'invalid event'
        -- ERROR
        else
            error 'invalid event'
